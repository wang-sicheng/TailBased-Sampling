...
public class BackendProcessData implements Runnable{
    ...
    private static AtomicInteger CURRENT_BATCH = new AtomicInteger(0);

    private static int BATCH_COUNT = 90;

    private static List<TraceIdBatch> TRACE_ID_BATCH_LIST= new ArrayList<>();
    ...
    public static boolean isFinished() {
        ...
        return FINISH_PROCESS_COUNT.get() >= Constants.PROCESS_COUNT;
    }

    public static TraceIdBatch getFinishedBatch() {
        int next = CURRENT_BATCH.get() + 1;
        if (next >= BATCH_COUNT) {
            next = 0;
        }
        TraceIdBatch nextBatch = TRACE_ID_BATCH_LIST.get(next);
        TraceIdBatch currentBatch = TRACE_ID_BATCH_LIST.get(CURRENT_BATCH.get());

        // when client process is finished, or then next trace batch is finished. to get checksum for wrong traces.
        boolean cond1 = FINISH_PROCESS_COUNT.get() >= PROCESS_COUNT && currentBatch.getBatchPos() > 0;
        ...
        if (cond1 || cond2) {
            ...
            TRACE_ID_BATCH_LIST.set(CURRENT_BATCH.get(), newTraceIdBatch);
            CURRENT_BATCH.set(next);
            return currentBatch;
        }
        return null;
    }
    ...
    public static String finish() {
        FINISH_PROCESS_COUNT.addAndGet(1);
